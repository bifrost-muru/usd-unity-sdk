...
namespace USD.NET.Unity
{
    [Serializable]
    [UsdSchema("Material")]
    public class MtlxMaterialSample : SampleBase
    {
        public MtlxMaterialSample() : base()
        {
        }

        public MtlxMaterialSample(string surfacePath) : base()
        {
            surface.SetConnectedPath(surfacePath);
        }

        public MtlxMaterialSample(string surfacePath, string displacementPath) : base()
        {
            surface.SetConnectedPath(surfacePath);
            displacement.SetConnectedPath(displacementPath);
        }

        [UsdNamespace("outputs:mtlx")]
        ...
        static public bool ReadMaterial(Scene scene,
            string geometryPath,
            MtlxMaterialSample MtlxMaterialSample,
            out string shaderId)
        {
            ...
            scene.Read(geomMaterialBinding.binding.targetPaths[0], MtlxMaterialSample);

            // Again, the surface may be invalid, so protect against broken scene description.
            if (MtlxMaterialSample.surface == null || string.IsNullOrEmpty(MtlxMaterialSample.surface.connectedPath))
            {
                throw new System.Exception("Material had no surface bound");
            }

            string shaderPath = MtlxMaterialSample.surface.connectedPath.Replace(".outputs:out", "");
